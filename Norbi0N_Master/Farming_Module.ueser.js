    ...
    const NorbiFarmEngine = {
        ...
        startFarming: async function() {
            ...
            const doFarming = async () => {
                self.isRunning = true;
                self.updateStatus('Opening farm tab...');
                self.log('Starting farm process');
                
                const currentHost = window.location.hostname;
                const farmUrl = `https://${currentHost}/game.php?village=${unsafeWindow.game_data.village.id}&screen=am_farm`;
                
                self.log('Opening farm tab: ' + farmUrl);
                self.farmTab = window.open(farmUrl, '_blank');
            ...
            if (typeof window.Norbi0N_QueueSystem !== 'undefined') {
                ...
                await window.Norbi0N_QueueSystem.requestSlot('farm', async () => {
                    self.log('✅ Got slot! Starting farming...');
                    await doFarming();
                    // Wait for farming to complete (monitored by monitorFarmTab)
                ...
                // Fallback: Run without queue system
